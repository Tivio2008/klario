...
export function HeroBlock({ content, theme, isPreview }: HeroBlockProps) {
  ...
  return (
    <section
      ...
      style={{ background: `radial-gradient(ellipse at 30% 20%, rgba(${rgb}, 0.35) 0%, transparent 55%), radial-gradient(ellipse at 70% 80%, rgba(${rgb}, 0.2) 0%, transparent 50%), #0a0a0f` }}
    >
      {/* Parallax orb layer */}
      <motion.div style={noAnim ? {} : { y: parallaxY }} className="absolute inset-0 pointer-events-none">
        ...
        />
        <motion.div
          animate={noAnim ? {} : { scale: [1, 1.1, 1], opacity: [0.08, 0.15, 0.08] }}
      ...
      >
        {content.badge && (
          <motion.div
            ...
            style={{ borderColor: `rgba(${rgb}, 0.4)`, backgroundColor: `rgba(${rgb}, 0.1)`, color: primary }}
          >
            <motion.span
        ...
        >
          <motion.button
            whileHover={noAnim ? {} : { scale: 1.05, boxShadow: `0 0 35px rgba(${rgb}, 0.55)` }}
          ...
          {whatsappUrl && (
            <motion.a
              ...
              whileTap={noAnim ? {} : { scale: 0.97 }}
              className="px-8 py-3.5 rounded-xl border border-green-500/40 bg-green-500/10 hover:bg-green-500/20 text-green-300 font-semibold text-lg transition-all flex items-center gap-2"
            >
